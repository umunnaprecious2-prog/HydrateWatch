...
- Backend is TypeScript + Express + Prisma. Switched from PostgreSQL to
  **CockroachDB** (`provider = "cockroachdb"` in schema.prisma) since
  that's the production target. This required changing every model's
  `@default(autoincrement())` to `@default(sequence())` (CockroachDB
  only allows autoincrement() on BigInt) and regenerating the migration
  for the CockroachDB SQL dialect (`STRING`/`INT4`/`GENERATED BY
  DEFAULT AS IDENTITY`) via `prisma migrate diff --from-empty` (offline,
  no live DB needed - and CockroachDB's SQL is NOT valid against plain
  Postgres, so the old Postgres migration is gone).
...
- **Not yet re-verified against CockroachDB itself**: a local CockroachDB
  container repeatedly failed to boot in this sandboxed session
  (`start-single-node` timing out on its own internal self-check -
  looked like a resource/timing issue specific to this environment, not
  a code problem). Docker Compose's `db` service was removed rather than
  switched to a flaky local CockroachDB container - `DATABASE_URL` now
  just points at a real CockroachDB cluster (Cloud Serverless free tier
  works for local dev too). **First real run against CockroachDB will
  be whenever the user deploys or points a local `.env` at a real
  cluster** - watch for CockroachDB-specific migration issues then.
